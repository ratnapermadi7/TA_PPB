...
function AllCats({ navigation }) {
  ...
  function getData() {
    const name = ['birman', 'persia', 'british']; // You can replace this with the dynamic value if needed
  ...
  return (
    <View style={{ flex: 1, marginTop: 20 }}>
      <FlatList
        data={data}
        renderItem={({ item }) => (
          <TouchableOpacity onPress={() => navigation.navigate('Detail All Cats', { msg: item.id })}>
